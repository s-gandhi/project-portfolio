        ...
        for fan in range(0, self.scans, self.fan_scans):
            ...
            for scan in range(fan + self.skip_scans, fan + self.fan_scans - self.skip_scans):
                if (scan < self.scans):
                    ...
                    reconstruction = np.where(reconstruction < -1024, -1024, reconstruction)
                    reconstruction = np.where( reconstruction > 3072, 3072, reconstruction)
                    plt.close()
                    draw(reconstruction)
                    ...
                    create_dicom(reconstruction, file, sp=scale, sz=scale, f=z, storage_directory='results',
                                 study_uid=studyuid,
                                 series_uid=seriesuid, time=time)
                    z = z + 1
