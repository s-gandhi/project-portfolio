...
def ct_detect(p, coeffs, depth, mas=10000):
    ...
    transmitted = np.random.poisson(detector_photons)
    background = np.random.poisson(bfactor)
    scattered = np.random.poisson(sfactor * np.sum(p))

    # add noise to detections
    detector_photons = transmitted + background + scattered
